...
##### 4.1 Set the `PYTHONPATH` environment variable to include the current working directory


Note : The `PYTHONPATH` is set to the current working directory to ensure that the scripts can find the necessary modules.
```bash
export ISAAC_PATH=~/isaacsim/isaac-sim
export PYTHONPATH=$PYTHONPATH:$ISAAC_PATH/exts/omni.isaac.lab:$ISAAC_PATH/extscache
export PYTHONPATH=$PYTHONPATH:$ISAAC_PATH/python/lib/python3.10/site-packages
...
```bash
python scripts/ftr_algo/train.py \
...
Play Mode for Multi-Agent Tasks
